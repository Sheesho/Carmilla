...
bot.on('messageUpdate', (oldMessage, newMessage) => {
   ...
   if((newMessage.content.startsWith(auth.prefix + 'emote')) && (!newMessage.author.bot))
	{
       {
		var bool = false;
		var cont = newMessage.content.substring(7,newMessage.length);
		urlExists(auth.emotes+cont+".png", function(err, exists)
		{
			if(exists)
			{
				...
				var promise = new Promise(function(resolve, reject) 
				{
					...
					resolve(newMessage.channel.send("", {file: auth.emotes + cont + ".png"}).catch(
					function() 
					{ 
		...
		urlExists(auth.emotes+cont+".gif", function(err, exists)
		{
			if(exists)
			{
				...
				{
					// do a thing, possibly async, then…
					resolve(newMessage.channel.send("", {file: auth.emotes + cont + ".gif"}).catch(
					function() 
					{ 
						console.log("promesse rompue");
					}));
